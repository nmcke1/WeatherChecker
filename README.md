...
Add a title so the tab in the browser has a proper name. In the HTML file, in the `<head>...</head>` section add a title tag:-
...
Next - add the CSS file to the web page. In the `<head>..</head>` section add this tag:-
...
So `getWeather()` should now look like this with the new line:-
